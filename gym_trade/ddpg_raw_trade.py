...
class Buffer:
    ...
    def update(
        ...
        # print("REWARD: ", tf.math.reduce_mean(reward_batch))
        # print("AVG REWARD: ", tf.math.reduce_mean(reward_batch))
        # print("Y: ", tf.math.reduce_mean(y))
        # print("CRITIC VALUE: ", tf.math.reduce_mean(critic_value))
...
for ep in range(total_episodes):
    ...
    episodic_reward = 0
    ep_step_index = 0
    while ep_step_index < max_steps:
        ep_step_index = ep_step_index + 1
...
env1 = TRADEEnv("TSLA","2016-01-01", "2022-01-01", 0.00)
obs = env1.reset()
obs = obs.flatten()
for i in range(15000):

    action, _states = policy(obs)
    print(action)
